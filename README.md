...
La entrega ejecutable está en `src/v2/`: consultas por hilo y DM, sesiones
persistentes, cola FIFO, stop, SQLite, consumo y entregas persistentes a Slack.
La primera implementación de fase 2 añade worktrees, tests, PRs, tareas en Notion,
salas privadas y revisión humana de plan/QA. Su aceptación con servicios reales
sigue pendiente. El checklist está en [docs/v2.md](docs/v2.md#101-avance-de-implementación).

```sh
pnpm run setup --migrate
pnpm test:v2
...
Antes de iniciar: revisa `config/regent.yaml` (ejemplo: `regent.example.yaml`), en
particular `auth.mode`, `slack.workspace_team_id` y `slack.allowed_users`. La migración
no consulta servicios ni activa Slack. Conserva un YAML existente y los enlaces de
`log/threads.json`; archiva `rooms.json` sin convertir terminales v1 en sesiones v2.
Sin API key ni usuarios previos, un YAML nuevo propone `indie` y requiere completar
tu ID de Slack antes de arrancar. Una configuración existente conserva su modo.

`pnpm start:v2` usa `REGENT_PORT=8788`, `REGENT_DB=log/v2.sqlite` y escucha solo en
