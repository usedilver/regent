...
export interface HistoryMessage { id: string; text: string; image?: ImageReference }
...
export class History {
  ...
  async prepare(runId: string, key: string, session: string | null, loader?: HistoryLoader, signal?: AbortSignal) {
    ...
    if (!row) return { text: '', images: [], acknowledge(_sessionId: string, _skipped?: Set<string>) {} }
    ...
    if (row.snapshot !== null) messages = JSON.parse(row.snapshot as string)
      ...
      messages = [...new Map(messages.map(m => [m.id, { id: m.id, text: redact(m.text), ...(m.image ? { image: m.image } : {}) }])).values()]
    ...
    const pending = messages.map(message => ({ ...message, hash: createHash('sha256').update(message.text + (message.image ? JSON.stringify(message.image) : '')).digest('hex') }))
    ...
    return {
      ...
      images: pending.filter(m => m.image).reverse().sort((a, b) => Number(b.id.startsWith(`${source.trigger}:image:`)) - Number(a.id.startsWith(`${source.trigger}:image:`))).map(m => ({ messageId: m.id, reference: m.image!, label: `[${m.id}] ${m.text}` })),
      acknowledge: (sessionId: string, skipped = new Set<string>()) => {
        for (const m of pending) if (!skipped.has(m.id)) this.store.db.prepare('INSERT INTO history_seen VALUES(?,?,?,?) ON CONFLICT(conversation_key,session_id,message_id) DO UPDATE SET hash=excluded.hash').run(key, sessionId, m.id, m.hash)
      },
    }
  }
}
